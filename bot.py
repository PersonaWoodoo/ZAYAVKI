...
def get_rules_inline_button():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📜 ПРАВИЛА ЧАТА", url=RULES_URL)]
    ])
...
class AnketaStates(StatesGroup):
    ...
    waiting_for_confirm = State()  # ЭТО СОСТОЯНИЕ ОБЯЗАТЕЛЬНО
...
async def send_accept_message(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"{EMOJI_CHECK} Отправлено @{username} с кнопкой правил")
...
async def main():
    ...
    print("🔘 Инлайн-кнопка с правилами добавлена в принятие")
