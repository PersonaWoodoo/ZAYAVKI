...
Q14 = f"{Q1}{Q4}"  # 1️⃣ и 4️⃣ вместе

# Остальные твои эмодзи
...
# Массив вопросов с их эмодзи
QUESTIONS = [
    (Q1, "Как тебя звать или как к тебе обращаться? (Имя/ник)"),
    (Q2, "Сколько тебе лет?"),
    (Q3, "Твой часовой пояс и город? (Для понимания времени активности)"),
    (Q4, "Сколько времени в день готов уделять модерации?"),
    (Q5, "Есть ли опыт модерации/администрирования? (Если да — кратко где и что делал)"),
    (Q6, "Сколько времени ты уже в чате UNIT CHAT?"),
    (Q7, "Почему хочешь стать админом?"),
    (Q8, "Ознакомлен ли ты с правилами UNIT CHAT?"),
    (Q9, "Представь: пользователь грубит, но правила не нарушает. Что сделаешь?"),
    (Q10, "Что ты сделаешь, если твой друг нарушил правило?"),
    (Q11, "Были ли у тебя конфликты с участниками или админами этого чата?"),
    (Q12, "Были ли у тебя нарушения и варны в чате UNIT CHAT?"),
    (Q13, "Укажи свой @username (или ID, если нет юзернейма)"),
...
def get_main_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"{EMOJI_WARNING} Проверить подписку", callback_data="check_subscribe")]
    ])
...
async def send_accept_message(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(
            user_id,
            f"{EMOJI_POPPER} <b>ВАША ЗАЯВКА ПРИНЯТА!</b> {EMOJI_GEM}\n\n"
        ...
        await message.answer(f"{EMOJI_CHECK} Отправлено @{username}")
...
async def send_reject_message(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(
            ...
            f"{admin_text}\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"{EMOJI_WARNING} Попробуй подать заявку позже. {EMOJI_SMILE}",
            parse_mode="HTML"
        )
        await message.answer(f"{EMOJI_CROSS} Отправлено @{username}")
    except Exception as e:
        await message.answer(f"{EMOJI_CROSS} Ошибка: {e}")
...
async def main():
    ...
    print("✨ 14 премиум-эмодзи для вопросов загружены")
